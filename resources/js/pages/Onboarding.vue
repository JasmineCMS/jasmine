...
<template>
  <AuthLayout :title="$t('auth.account_setup')">
    <p class="text-white/80 text-sm mb-5" v-text="$t('auth.choose_password_to_activate')" />

    <Form method="POST" :action="action" #default="{processing, errors}" class="space-y-5">
      <!-- Email -->
      <div class="group">
        <label
      ...
      >
        <span v-if="!processing" v-text="$t('auth.activate_account')" />
