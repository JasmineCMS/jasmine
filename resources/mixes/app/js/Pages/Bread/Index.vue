    ...
    <div class="px-1 px-lg-4">
      <DataTable :paginator="paginator" :columns="columns" ref="dt" :searchable="searchable" @reordered="reorder">
        <template #tableActions>
          <div class="d-flex">
            <button
              ...
              type="button"
              @click="fake"
              class="btn text-info d-flex align-items-center">
        ...
        <template #h_j_sort="{col, q}">
          <div>
            <div style="cursor: pointer" v-text="col.label" @click="initSort(col, q)" />
        ...
        <template #j_actions="{v, r}">
          <div class="d-flex align-items-center">
            <a v-if="r.jasmine_public_url" target="_blank" :href="r.jasmine_public_url" :title="$t('View')" class="btn">
            ...
            <button v-if="can.indexOf('d') > -1" type="button" class="btn" :title="$t('Delete')" @click="del(v, r)">
...
export default {
  ...
  methods: {
    ...
    async reorder(rows) {
      ...
      this.$inertia.put(this.route('jasmine.bread.reorder', {breadableName: this.b.key}), {order: new_order});
    },
    ...
    async fake() {
      ...
      this.$inertia.post(
        this.route('jasmine.bread.fake', {breadableName: this.b.key}),
        {count: value},
        {
          preserveState: false,
        },
