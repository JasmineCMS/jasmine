          ...
          <div class="modal-body p-0">
            ...
            <div class="text-center my-2">
              <button
                type="button"
                @click="saveCrop"
                class="btn btn-primary btn-sm rounded-1 mx-auto px-3"
                v-text="$t('Save')" />
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async upload(imgFile) {
      ...
      if (!checkRes.ok)
        await fetch(vm.route('fm.create-directory'), {
          method: 'post',
  ...
  watch: {
    ...
    showCropper(v) {
      v ? (document.body.style.overflow = 'hidden') : (document.body.style.overflow = 'initial');
    },
  },
