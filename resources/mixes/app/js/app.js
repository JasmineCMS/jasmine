...
(async function () {
  ...
  let Ziggy = await Vue.reactive({
    async reload() {
      let r = await fetch(document.head.querySelector('meta[name="routes"]').content).then(r => r.json());
  ...
  let globals = await Vue.reactive({
    async reload() {
      let r = await fetch(document.head.querySelector('meta[name="globals"]').content).then(r => r.json());
  ...
  createInertiaApp({
    ...
    setup({el, App, props, plugin}) {
      ...
      window.app.config.globalProperties.$globals = globals;
      window.app.config.globalProperties.$fixed = window[fixed];

      fetch(document.head.querySelector('meta[name="locale"]').content)
        .then(r => r.json())
        .then(m => {
          i18n.global.setLocaleMessage(document.documentElement.lang, m);
          document.dispatchEvent(new CustomEvent('jasmine:beforeMount', {detail: {app: window.app}}));
          window.app.mount(el);
        });
    },
  });
